...
def add_common_args(parser):
    parser.add_argument("--in-norms", type=str_list, help="comma-sep. list of input normalization functions")
    parser.add_argument("--tgt-norms", type=str_list, help="comma-sep. list of target normalization functions")
    parser.add_argument(
        "--crop",
        type=int_tuple,
    ...
    parser.add_argument("--criterion", default="MSELoss", type=str, help="loss function")
    parser.add_argument(
        "--load-state",
        default=ckpt_link,
        type=str,
    ...
    parser.add_argument(
        "--batch-size", "--batches", type=int, required=True, help="mini-batch size, per GPU in training or in total in testing"
    )
    parser.add_argument(
        "--loader-workers", default=8, type=int, help="number of subprocesses per data loader. 0 to disable multiprocessing"
    )

    parser.add_argument(
        "--callback-at",
        type=lambda s: Path(s).resolve(),
        help="directory of custom code defining callbacks for models, "
...
def add_train_args(parser):
    ...
    parser.add_argument(
        "--train-in-patterns", type=str_list, required=True, help="comma-sep. list of glob patterns for training input data"
    )
    parser.add_argument(
        "--train-tgt-patterns", type=str_list, required=True, help="comma-sep. list of glob patterns for training target data"
    )
    parser.add_argument("--train-style-pattern", type=str, help="glob pattern for training data styles")
    parser.add_argument("--val-in-patterns", type=str_list, help="comma-sep. list of glob patterns for validation input data")
    parser.add_argument("--val-tgt-patterns", type=str_list, help="comma-sep. list of glob patterns for validation target data")
    parser.add_argument("--val-style-pattern", type=str, help="glob pattern for validation data styles")
    parser.add_argument("--augment", action="store_true", help="enable data augmentation of axis flipping and permutation")
    parser.add_argument(
        "--aug-shift",
        type=int_tuple,
    ...
    parser.add_argument("--aug-add", type=float, help="additive data augmentation, (normal) std, same factor for all fields")
    parser.add_argument(
        "--aug-mul", type=float, help="multiplicative data augmentation, (log-normal) std, same factor for all fields"
    )

    parser.add_argument("--adv-model", type=str, help="discriminator model, disabled by default")
    parser.add_argument(
        "--adv-model-spectral-norm", action="store_true", help="enable spectral normalization on the discriminator"
    )
    parser.add_argument("--adv-criterion", default="WDistLoss", type=str, help="adversarial loss function")
    parser.add_argument(
    ...
    parser.add_argument("--adv-start", default=0, type=int, help="epoch to start adversarial training")
    parser.add_argument("--adv-iter-ratio", default=1, type=int, help="discriminator iterations per generator update")
    parser.add_argument(
        "--adv-label-smoothing",
        default=1,
        type=float,
        help="label of real samples for the discriminator, e.g. 0.9 for label smoothing and 1 to disable",
    )
    parser.add_argument(
        "--instance-noise", default=0, type=float, help="noise added to the adversary inputs to stabilize training"
    )
    parser.add_argument("--instance-noise-batches", default=1e4, type=float, help="noise annealing duration")
    ...
    parser.add_argument("--adv-lr", type=float, help="initial adversary learning rate, default to --lr")
    parser.add_argument(
        "--adv-optimizer-args", type=json.loads, help="adversary optimizer arguments, default to --optimizer-args"
    )
    parser.add_argument("--reduce-lr-on-plateau", action="store_true", help="Enable ReduceLROnPlateau learning rate scheduler")
    parser.add_argument(
        "--scheduler-args", default='{"verbose": true}', type=json.loads, help="arguments for the ReduceLROnPlateau scheduler"
    )
    parser.add_argument("--init-weight-std", type=float, help="weight initialization std")
    parser.add_argument("--epochs", default=128, type=int, help="total number of epochs to run")
    ...
    parser.add_argument("--dist-backend", default="nccl", type=str, choices=["gloo", "nccl"], help="distributed backend")
    parser.add_argument("--log-interval", default=100, type=int, help="interval (batches) between logging training loss")
    parser.add_argument("--detect-anomaly", action="store_true", help="enable anomaly detection for the autograd engine")
...
def add_test_args(parser):
    ...
    parser.add_argument(
        "--test-in-patterns", type=str_list, required=True, help="comma-sep. list of glob patterns for test input data"
    )
    parser.add_argument(
        "--test-tgt-patterns", type=str_list, required=True, help="comma-sep. list of glob patterns for test target data"
    )
    parser.add_argument("--test-style-pattern", type=str, help="glob pattern for test data styles")
...
def int_tuple(s: str) -> int | tuple[int, ...]:
    t = s.split(",")
