import os
from collections.abc import Iterator

import torch
import torch.distributed as dist
from torch.utils.data import Sampler

from cs_5960_ast.map2map.src.data.fields import FieldDataset
from cs_5960_ast.methods.utils import str_to_bool


class DistFieldSampler(Sampler):
    """Distributed sampler for field data, useful for multiple crops

    Stochastic training on fields with multiple crops puts burden on the IO.
    A node may load files of the whole field but only need a small part of it.
    Numpy memmap can load part of the field, but can also be very slow (even
    slower than reading the whole thing)

    `div_data` enables data file division among GPUs when `shuffle=True`.
    For field with multiple crops, it helps IO by benefiting from the page
    cache, but limits stochasticity.
    Increase `div_shuffle_dist` can mitigate this by shuffling the order of
    samples within the specified distance.

    When `div_data=False` this sampler behaves similar to `DistributedSampler`,
    except for the chunky (rather than strided) subsample slicing.
    Like `DistributedSampler`, `set_epoch()` should be called at the beginning
    of each epoch during training.

    NOTE: seems to quietly drop the last datapoints if dataset is not evenly divisible among processes
    """

    def __init__(
        self,
        dataset: FieldDataset,
        *,
        shuffle: bool,
        div_data: bool = False,
        div_shuffle_dist: int = 0,
        weighted_sample: bool = False,
    ) -> None:
        dist_status = str_to_bool(os.getenv("DISTRIBUTED", default=None))

        if dist_status:
            self.rank = dist.get_rank()
            self.world_size = dist.get_world_size()
        else:
            self.rank = 0
            self.world_size = 1

        self.dataset: FieldDataset = dataset
        if len(dataset) <= 0:
            msg = "Dataset has no length, sampler has no samples to extract"
            raise ValueError(msg)
        self.nsample: int = len(dataset)
        self.nfile = dataset.nfile
        self.ncrop = dataset.ncrop
        # print(self.nsample, self.nfile, self.ncrop)

        self.shuffle = shuffle

        self.div_data = div_data
        self.div_shuffle_dist = div_shuffle_dist

    def __iter__(self) -> Iterator[float]:
        if self.shuffle:
            # deterministically shuffle based on epoch
            g = torch.Generator()
            g.manual_seed(self.epoch)

            if self.div_data:
                # shuffle files
                ind: torch.Tensor = torch.randperm(self.nfile, generator=g)
                ind: torch.Tensor = ind[:, None] * self.ncrop + torch.arange(self.ncrop)
                ind: torch.Tensor = ind.flatten()

                # displace crops with respect to files
                dis = torch.rand((self.nfile, self.ncrop), generator=g) * self.div_shuffle_dist
                loc = torch.arange(self.nfile)
                loc = loc[:, None] + dis
                loc = loc.flatten() % self.nfile  # periodic in files
                dis_ind = loc.argsort()

                # shuffle crops
                ind: list[float] = ind[dis_ind].tolist()
            else:
                ind: list[float] = torch.randperm(self.nsample, generator=g).tolist()
        else:
            ind: list[float] = list(range(self.nsample))

        # cap with length of ind to prevent index out of range errors
        # will always select 1 datapoint, can be the single datapoint possible if len(ind)==1
        start: int = min(len(ind) - 1, self.rank * len(self))
        stop: int = min(len(ind), start + len(self))
        ind: list[float] = ind[start:stop]

        return iter(ind)

    def __len__(self) -> int:
        # if less samples than processes, len is 0 which breaks dataloader
        # even if each process must use the same file, we should have at least 1 data point everywhere
        return max(1, self.nsample // self.world_size)

    def set_epoch(self, epoch: int) -> None:
        self.epoch = epoch
