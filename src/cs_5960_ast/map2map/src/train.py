...
from .utils import import_attr, load_model_state_dict, plt_power, plt_slices, score
...
def local_train(args):
    ...
    args.gpus_per_node = max(args.gpus_per_node, 1)  # can't have 0 devices that makes no sense
    print(f"{args.gpus_per_node=}")
    args.nodes = 1
    args.world_size = args.nodes * args.gpus_per_node
    print(f"{args.world_size=}")

    print("INFO: training locally")
    # spawn(gpu_worker, args=(0, args), nprocs=args.gpus_per_node)
    gpu_worker(local_rank=0, node=0, args=args)
...
def node_worker(args):
    if "SLURM_STEP_NUM_NODES" in os.environ:
        ...
        msg = "missing node counts in slurm env"
        raise KeyError(msg)
    args.gpus_per_node = torch.cuda.device_count()
    args.world_size = args.nodes * args.gpus_per_node
...
def gpu_worker(local_rank, node, args):
    ...
    # for local training
    remote_node_status = os.getenv("REMOTE_NODE", None)
    ...
    train_sampler = DistFieldSampler(
        train_dataset, shuffle=True, div_data=args.div_data, div_shuffle_dist=args.div_shuffle_dist
    )
    # random_sampler =
    ...
    if args.val:
        ...
        val_sampler = DistFieldSampler(
            val_dataset, shuffle=False, div_data=args.div_data, div_shuffle_dist=args.div_shuffle_dist
        )
    ...
    model = model(
        sum(args.in_chan), sum(args.out_chan), style_size=args.style_size, scale_factor=args.scale_factor, **args.misc_kwargs
    )
    model.to(device)
    ...
    if args.adv:
        ...
        adv_model.to(device)
        adv_model = DistributedDataParallel(adv_model, device_ids=[device], process_group=dist.new_group())
        ...
        adv_scheduler = optim.lr_scheduler.ReduceLROnPlateau(adv_optimizer, **args.scheduler_args)
    ...
    if (args.load_state == ckpt_link and not os.path.isfile(ckpt_link)) or not args.load_state:
        ...
        load_model_state_dict(model.module, state["model"], strict=args.load_state_strict)
        ...
        if args.adv:
            if "adv_model" in state:
                load_model_state_dict(adv_model.module, state["adv_model"], strict=args.load_state_strict)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.val:
            val_loss = validate(epoch, val_loader, model, criterion, adv_model, adv_criterion, logger, device, args)
            # epoch_loss = val_loss
        ...
        if rank == 0:
            ...
            if (min_loss is None or epoch_loss[0] < min_loss[0]) and epoch >= args.adv_start:
                min_loss = epoch_loss
...
def train(
    ...
    if args.log_interval <= args.adv_wgan_gp_interval or args.adv_wgan_gp_interval < 1:
        ...
        adv_wgan_gp_log_interval = args.log_interval // args.adv_wgan_gp_interval * args.adv_wgan_gp_interval
    ...
    adv_real = torch.full([1], args.adv_label_smoothing, dtype=torch.float32, device=device)
    ...
    for i, data in enumerate(loader):
        ...
        if batch % args.log_interval == 0:
            ...
            if rank == 0:
                ...
                if args.adv and epoch >= args.adv_start:
                    logger.add_scalar("loss/batch/train/adv/G", loss_adv.item(), global_step=batch)
                    logger.add_scalars(
                ...
                if args.adv and epoch >= args.adv_start:
                    logger.add_scalar("grad/adv/first", adv_grads[0], global_step=batch)
                    logger.add_scalar("grad/adv/last", adv_grads[-1], global_step=batch)

                    if noise_std > 0:
                        logger.add_scalar("instance_noise", noise_std, global_step=batch)
    ...
    if rank == 0:
        ...
        if args.adv and epoch >= args.adv_start:
            logger.add_scalar("loss/epoch/train/adv/G", epoch_loss[1], global_step=epoch + 1)
            logger.add_scalars(
...
def validate(epoch, loader, model, criterion, adv_model, adv_criterion, logger, device, args):
    model.eval()
    if args.adv:
        adv_model.eval()
...
def set_requires_grad(module, requires_grad=False) -> None:
