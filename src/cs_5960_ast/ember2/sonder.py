"""File kept for reference, but is not directly used and has no implemented functionality"""
...
class sonder:
    def __init__(self, cfg):
        self.solve_env(cfg)
        self.nn_base = Ember2Net(self.config, mode="base")
        self.nn_zoom = Ember2Net(self.config, mode="zoom")
    ...
    def solve_env(self, cfg):
        ...
        paths = [self.run_dir + "/base", self.run_dir + "/zoom"]
        for path in paths:
    ...
    def load_ckpt(self, set_nr, mode, ckpt=None):
        ...
        if ckpt is None:
            ckpt = max(glob.glob(pdir + "ckpt_*.pt"), key=os.path.getctime).split("ckpt_")[-1].strip(".pt")
    ...
    def train(self, set_nr, mode):
        ...
        dataloader = DataLoader(
            dataset=MemmapDataset(self.config, set_nr), batch_size=self.config["global_bs"], num_workers=os.cpu_count()
        )
        ...
        for epoch in range(10000):
            ...
            for batch in tqdm.tqdm(dataloader):  # train_ds
                nn.train_step(step, transfer_to(batch, self.device), logger)
